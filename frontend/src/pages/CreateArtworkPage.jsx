...
export default function CreateArtworkPage() {
  ...
  useEffect(() => {
    ...
    const loadArtwork = async () => {
      try {
        setPageLoading(true);
        const response = await fetch(`${API_URL}/api/content/${id}?type=artwork`, {
          headers: {
        ...
        if (!data.success) {
          setError(data.error?.message || 'Failed to load artwork');
          return;
        }
  ...
  const handleFileChange = async (e) => {
    ...
    if (imageItems.length + files.length > 10) {
      setError('You can upload up to 10 images per artwork.');
      e.target.value = '';
      return;
    }

    const validation = await validateImageFilesBeforeUpload(files, {
      maxFiles: 10,
    ...
    setImageItems((prev) => [...prev, ...createFileImageItems(files)]);
    e.target.value = '';
  };
  ...
  const submitArtwork = async (status) => {
    setError('');
    setLoading(true);
    ...
    try {
      ...
      if (!isEditMode && imageItems.length === 0) {
        setError('Please upload at least one image from your device');
        setLoading(false);
        return;
      }

      if (isEditMode && imageItems.length === 0) {
        setError('Please keep at least one image or upload a new file');
        setLoading(false);
        return;
      }
      ...
      const response = await fetch(isEditMode ? `${API_URL}/api/content/${id}` : `${API_URL}/api/artworks`, {
      ...
      <form onSubmit={handleSubmit} className="panel p-6">
          ...
          <div className="mt-4 space-y-3">
            <div className="rounded-[1.75rem] border border-slate-800 bg-slate-950/50 p-4">
              ...
              <ImageOrderPicker
                items={imageItems}
                apiUrl={API_URL}
                emptyLabel="No artwork images selected yet. Upload files to build the order before publishing."
                helperText="Drag cards to reorder quickly. The first card becomes the cover image users see first."
        ...
        <div className="mb-6">
          ...
          <input
            ...
            className="input-base"
            placeholder="digitalart anime landscape"
            required
          />
          <p className="text-xs text-slate-400 mt-2">
        ...
        <div className="flex gap-4">
          ...
          >
            {loading ? 'Uploading...' : isEditMode ? 'Update Artwork' : 'Upload Artwork'}
          </button>
          <button
            type="button"
            onClick={() => navigate(isEditMode ? `/artwork/${id}` : '/artworks')}
            className="editor-action-secondary"
          >
            Cancel
          </button>
